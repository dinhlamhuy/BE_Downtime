            ...
            for (const projectDeps of perProjectResult.scannedProjects) {
                try {
                    ...
                    const targetFileRelativePath = projectDeps.plugin.targetFile ||
                        ...
                        '';
                    const res = await promiseOrCleanup(monitor_1.monitor(path, generateMonitorMeta(options, extractedPackageManager, projectName), projectDeps, options, projectDeps.plugin, targetFileRelativePath, contributors, generateProjectAttributes(options), generateTags(options)), spinner_1.spinner.clear(postingMonitorSpinnerLabel));
                    res.path = path;
...
function generateMonitorMeta(options, packageManager, projectName) {
    return {
        method: 'cli',
        packageManager,
        'policy-path': options['policy-path'],
        'project-name': options['project-name'] || projectName || config_1.default.PROJECT_NAME,
